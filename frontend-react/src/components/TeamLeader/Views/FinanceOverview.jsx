...
const FinanceOverview = () => {
    ...
    // Admin Settings Module style tab state
    ...
    const fetchFinanceData = async () => {
        try {
            ...
            const [expenseData, attendanceData] = await Promise.all([
                get('/teamleader/expenses/'),
                get('/teamleader/attendance/')
            ]);
            setTeamExpenses(expenseData || []);
            setAttendance(Array.isArray(attendanceData) ? attendanceData : []);
        } catch (err) {
            console.error('Error loading team finance data:', err);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <section className="view-section active fade-in">
            <h2 className="gradient-text" style={{ marginBottom: '1.5rem' }}>Team Finance Overview</h2>
