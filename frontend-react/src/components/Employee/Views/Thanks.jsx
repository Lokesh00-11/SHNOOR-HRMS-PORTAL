...
const Thanks = ({ currentMode }) => {
    ...
    useEffect(() => {
        const fetchEmployeesAndProfile = async () => {
            ...
            try {
                const profileData = await get('/employee/profile/');
        ...
        const fetchFeed = async () => {
            try {
                const feedData = await get('/employee/thanks/');
                setFeed(feedData || []);
            } catch (e) { }
        };
    ...
    const getStepperStyle = (stepNum) => ({
        ...
        textAlign: 'center',
        padding: '0.75rem',
        background: modalStep === stepNum ? 'rgba(253, 254, 255, 1)' : 'transparent',
        color: modalStep === stepNum ? 'var(--primary)' : '#666666',
        borderBottom: `2px solid ${modalStep === stepNum ? 'var(--primary)' : '#e5e7eb'}`,
    ...
    return (
        <section className="view-section active">
            ...
            <p style={{ color: 'var(--text-muted)', marginBottom: '2rem' }}>
                Thank badges are a way to appreciate colleagues for their hard work, dedication, or special contributions.            </p>
            ...
            {showModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
                    <div className="glass-panel" style={{ width: '100%', maxWidth: '800px', background: '#ffffff', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
                            ...
                            <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: '1.5rem', textAlign: 'center' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem' }}>
                                    {(() => {
                                        ...
                                        const now = new Date();
                                        feed.forEach(item => {
                                            if (item.title === infoBadge.title) {
                                                total++;
                                                let d = new Date();
                                                if (item.created_at) {
                                                    // safely parse whatever date string comes in
                                                    const cleanDate = String(item.created_at).trim();
                                                    // If it's DD-MM-YYYY (optionally with time)
                                                    if (cleanDate.match(/^\d{1,2}-\d{1,2}-\d{4}/)) {
                                                        const dateOnly = cleanDate.split('T')[0].split(' ')[0]; // isolate DD-MM-YYYY
                                        ...
                                        return (
                                            <>
                                                <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', width: '120px', padding: '1rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                                    <div style={{ fontSize: '1.5rem', color: '#333', marginBottom: '0.5rem' }}>{thisMonth}</div>
                                                    <div style={{ fontSize: '0.8rem' }}>This Month</div>
                                                </div>
                                                <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', width: '120px', padding: '1rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                                    <div style={{ fontSize: '1.5rem', color: '#333', marginBottom: '0.5rem' }}>{total}</div>
