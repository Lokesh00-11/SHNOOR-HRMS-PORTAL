...
const PayrollFinancePanel = () => {
    ...
    const fetchPayroll = async () => {
        try {
            setLoading(true);
            const employees = await get('/manager/employees/');
            
            const currentMonth = new Date().toLocaleString('default', { month: 'long', year: 'numeric' });
            const formattedData = (employees || []).map(emp => {
                const empSalary = parseFloat(emp.salary) || 55000.00;
                
                // Extract name safely from ManagerEmployeeListView
                ...
                return {
                    ...
                    salary: empSalary,
                    credited: true,
                    amount_credited: empSalary,
                    payment_date: new Date().toLocaleDateString('en-GB') + ' ' + new Date().toLocaleTimeString('en-GB')
                };
            });
    ...
    const handlePaySalary = async (employeeId) => {
        alert('As a manager, you have read-only visibility into payroll. Only Admin or HR can process actual payments.');
    };
